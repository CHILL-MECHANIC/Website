...
// ===== JWT-based auth token verification =====
// Verifies tokens generated by our auth endpoint (Supabase-compatible JWT format)
...
export function verifyAuthToken(authHeader: string | undefined): TokenVerifyResult {
  if (!authHeader?.startsWith('Bearer ')) {
    return { user: null, error: 'Authorization required' };
  }
  ...
  try {
    const decoded = jwt.verify(token, jwtSecret) as _JWTPayload;
